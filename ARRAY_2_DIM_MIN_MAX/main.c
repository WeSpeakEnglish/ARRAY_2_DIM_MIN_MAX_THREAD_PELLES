...
volatile uint16_t position;                              //position inside the buffer
 ...
 int amp1 = MIDDLE_VALUE;
 int amp2 = MIDDLE_VALUE;

int module (int A){
 return (A < 0)? -A: A;
}
 int ApmControl(int part){
     int amp1ch = 0;
     int amp2ch = 0;
     int amp1max = 0;
     int amp2max = 0;
 ...
 if (amp1max > THRESHOLD_VALUE) amp1--;
 else amp1++;
 if (amp2max > THRESHOLD_VALUE) amp2--;
 else amp2++;
...
DWORD WINAPI DoStuff(LPVOID lpParameter)
{
    ...
    Sleep(2000);
     //fill with sin
     *pointerCH1 = MIDDLE_VALUE + (uint16_t)(float)amp1*sin((float)counterThread/2.0);
     pointerCH1+=2;
     counterArray++;
 //    pointerCH1++;
     if (pointerCH1 > &bufferADC[BUFFER_SIZE -1][0]) pointerCH1 = &bufferADC[0][0];

     *pointerCH2 = MIDDLE_VALUE + (uint16_t)(float)amp1*sin((float)counterThread/2.7 + 0.17);
     pointerCH2+=2;
     counterArray++;
